...
void ViewerAR::Run()
{
    ...
    while(1)
    {
        GetImagePose(im,Tcw,status,vKeys,vMPs,Tcm,a,vIndices);
        if(im.empty())
            cv::waitKey(mT);
        else
        {
    ...
    pangolin::Var<std::string> menu_NUMBER("menu.INPUT 1-9","number");
    pangolin::Var<bool> menu_detectplane("menu.Insert Cube",false,false);

    pangolin::Var<bool> menu_ONOFF("menu.ONOFF",true,true);
    pangolin::Var<bool> menu_clear("menu.Clear All",false,false);
    pangolin::Var<bool> menu_drawim("menu.Draw Image",false,false);
    pangolin::Var<bool> menu_drawArea("menu.DrawArea", false,true);
    pangolin::Var<bool> menu_drawcube("menu.Draw Cube",true,true);
    pangolin::Var<float> menu_cubesize("menu. Cube Size",0.01,0.01,0.3);
    pangolin::Var<bool> menu_drawgrid("menu.Draw Grid",true,true);
    pangolin::Var<int> menu_ngrid("menu. Grid Elements",2,1,10);
    pangolin::Var<float> menu_sizegrid("menu. Element Size",0.02,0.01,0.3);
    ...
    pangolin::Var<bool> menu_Recompute("menu.Recompute", false, false);
    pangolin::Var<bool> menuSaveMap("menu.SaveMap",false,false);
    pangolin::Var<bool> menuReset("menu.Reset",false,false);
    pangolin::Var<bool> menu_SaveARPos("menu.SaveARPos", false,false);
    pangolin::Var<bool> menu_LoadARPos("menu.LoadARPos", false,false);
    ...
    pangolin::View& d_cam = pangolin::CreateDisplay()
            .SetBounds(0.0, 1.0, pangolin::Attach::Pix(wui), 1.0, -1024.0f/768.0f)
            .SetHandler(new pangolin::Handler3D(s_cam));
//     pangolin::OpenGlMatrix Twc;
//     Twc.SetIdentity();

//     pangolin::View& d_image = pangolin::Display("image")
//             .SetBounds(1/2.0f,1.0f,pangolin::Attach::Pix(wui),1/2.0f,(float)w/h)
...
//             .SetLock(pangolin::LockLeft, pangolin::LockTop);
    ...
    //更新1-9模型位姿 键盘ctrl+数字1-9
    pangolin::RegisterKeyPressCallback('1', pangolin::SetVarFunctor<std::string>("menu.INPUT 1-9", "1"));
    pangolin::RegisterKeyPressCallback('2', pangolin::SetVarFunctor<std::string>("menu.INPUT 1-9", "2"));
    pangolin::RegisterKeyPressCallback('3', pangolin::SetVarFunctor<std::string>("menu.INPUT 1-9", "3"));
    pangolin::RegisterKeyPressCallback('4', pangolin::SetVarFunctor<std::string>("menu.INPUT 1-9", "4"));
    pangolin::RegisterKeyPressCallback('5', pangolin::SetVarFunctor<std::string>("menu.INPUT 1-9", "5"));
    pangolin::RegisterKeyPressCallback('6', pangolin::SetVarFunctor<std::string>("menu.INPUT 1-9", "6"));
    pangolin::RegisterKeyPressCallback('7', pangolin::SetVarFunctor<std::string>("menu.INPUT 1-9", "7"));
    pangolin::RegisterKeyPressCallback('8', pangolin::SetVarFunctor<std::string>("menu.INPUT 1-9", "8"));
    pangolin::RegisterKeyPressCallback('9', pangolin::SetVarFunctor<std::string>("menu.INPUT 1-9", "9"));
    //其他操作
    pangolin::RegisterKeyPressCallback('i', pangolin::SetVarFunctor<bool>("menu.Insert Cube", true));
    pangolin::RegisterKeyPressCallback('c', pangolin::SetVarFunctor<bool>("menu.Clear All", true));
    pangolin::RegisterKeyPressCallback('r', pangolin::SetVarFunctor<bool>("menu.Reset", true));
    pangolin::RegisterKeyPressCallback('s', pangolin::SetVarFunctor<bool>("menu.SaveMap", true));
    pangolin::RegisterKeyPressCallback('l', pangolin::SetVarFunctor<bool>("menu.Localization Mode", true));
    pangolin::RegisterKeyPressCallback(pangolin::PANGO_CTRL + 'l', pangolin::SetVarFunctor<bool>("menu.Localization Mode", false));
    pangolin::RegisterKeyPressCallback('o', pangolin::SetVarFunctor<bool>("menu.ONOFF", true));
    pangolin::RegisterKeyPressCallback('f', pangolin::SetVarFunctor<bool>("menu.ONOFF", false));
    pangolin::RegisterKeyPressCallback('d', pangolin::SetVarFunctor<bool>("menu.Draw Cube", true));
    pangolin::RegisterKeyPressCallback(pangolin::PANGO_CTRL + 'd', pangolin::SetVarFunctor<bool>("menu.Draw Cube", false));
    pangolin::RegisterKeyPressCallback('a', pangolin::SetVarFunctor<std::string>("menu.INPUT 1-9", "number"));
    ...
    while(ni>-1)
    {
	    ...
	    //将局部地图点取出
	    int nn = vIndices.size();
        mvMPsInArea = vector<MapPoint*>(nn,static_cast<MapPoint*>(NULL));
	    if(!vIndices.empty())
        {
            int i2 = 0;
        ...
        //判断输入框中输入的是否为数字
        if (IsNumber(menu_NUMBER))
        {
            int i =std::stoi(menu_NUMBER);
            mnumber=i;
            if(menu_ONOFF)
            {
                mvbDraw[mnumber] = true;
            }else{
                mvbDraw[mnumber] = false;
            }
            menu_drawArea=true;
        }
        else{
        ...
        if(menu_drawArea)
        {
            ...
            pangolin::SetVarFunctor<std::string>("menu.NUMBER", "number");
        }
        ...
        if(status==2)
        {
            if(menu_clear)
            {
                if(!mvpPlane.empty())
                {
//                    for(size_t i=0; i<mvpPlane.size(); i++)
//                    {
//                        delete mvpPlane[i];
//                    }
//                    mvpPlane.clear();
            ...
            if(menu_detectplane)
            {
                std::vector<ORB_SLAM2::MapPoint*> vMPstemp = mvMPsInArea;
                Plane* pPlane = DetectPlane(Tcw,vMPstemp,50);
                if(pPlane)
                {
                    cout << "New virtual cube inserted!" << endl;
                    //mvpPlane.push_back(pPlane);
                    cout<<"mnumber: "<<mnumber<<endl;
                    mvpPlane[mnumber]=pPlane;
                }
                else
                {
            ...
            if(!mvpPlane.empty())
            {
                ...
                if(menu_Recompute)
                {
                    cout << "All virtual elements are recomputed!" << endl;
                    bRecompute = true;
        ...
        if(menuReset)
        {
            ...
            bLocalizationMode = false;
            menu_drawgrid = true;
            menu_drawpoints = false;
            mpSystem->Reset();
            menuReset = false;
            menu_LocalizationMode = false;
	    }
        //增加的部分：地图保存*************************************************************************************************
        if(menuSaveMap)
	    {
        ...
        {
            SaveARPos(sTpwPath);
            menu_SaveARPos = false;
        }

        if(menu_LoadARPos)
        {
            LoadARPos(sTpwPath);
            menu_LoadARPos = false;
        }

        if(menu_GetTpw)
        {
            mpSystem->PlaneSwitch();
            mTpw = mpSystem->GetPlaneMatrix();

            cout<<"Tpw = "<< endl << mTpw << endl;
        ...
        if(menu_drawcube)
        {
            ...
            {
                glPushMatrix();
                pangolin::OpenGlMatrix glT = TransformToGLMatrix(mTpw);
                glT.Multiply();
                DrawCube(menu_cubesize);
                glPopMatrix();
            }
...
void ViewerAR::SetImagePose(const cv::Mat &im, const cv::Mat &Tcw, const int &status, const vector<cv::KeyPoint> &vKeys, 
			    const vector<ORB_SLAM2::MapPoint*> &vMPs, const cv::Mat &Tcm, const int& a, const vector<size_t > &vIndices)
{
...
void ViewerAR::SetImagePose(const cv::Mat &im, const cv::Mat &Tcw, const int &status, const vector<cv::KeyPoint> &vKeys,
                            const vector<ORB_SLAM2::MapPoint*> &vMPs, const cv::Mat &Tcm, const int& a)
{
    unique_lock<mutex> lock(mMutexPoseImage);
    mImage = im.clone();
    mTcw = Tcw.clone();
...
void ViewerAR::SetTfixedFilePath(const string &sTfixedPath, const string &sTcw1Path)
{
    msTfixedPath = sTfixedPath;
    msTcw1Path = sTcw1Path;
}

void ViewerAR::GetTfixedFilePath(string &sTfixedPath, string &sTcw1Path)
{
    sTfixedPath = msTfixedPath;
    sTcw1Path = msTcw1Path;
}

void ViewerAR::SetTpwPath(const string &sTpwPath)
{
    msTpwPath = sTpwPath;
}

void ViewerAR::GetTpwPath(string &sTpwPath)
{
    sTpwPath = msTpwPath;
}
//***************************************************************************************************************

pangolin::OpenGlMatrix ViewerAR::TransformToGLMatrix(const cv::Mat &T)
{
    pangolin::OpenGlMatrix glT;
    glT.m[0] = T.at<float>(0,0);
    glT.m[1] = T.at<float>(1,0);
    glT.m[2] = T.at<float>(2,0);
    glT.m[3] = 0.0;
    
    glT.m[4] = T.at<float>(0,1);
    glT.m[5] = T.at<float>(1,1);
    glT.m[6] = T.at<float>(2,1);
    glT.m[7] = 0.0;
    
    glT.m[8] = T.at<float>(0,2);
    glT.m[9] = T.at<float>(1,2);
    glT.m[10] = T.at<float>(2,2);
    glT.m[11] = 0.0;
    
    glT.m[12] = T.at<float>(0,3);
    glT.m[13] = T.at<float>(1,3);
    glT.m[14] = T.at<float>(2,3);
    glT.m[15] = 1.0;
...
void ViewerAR::DrawCube(const float &size,const float x, const float y, const float z)
{
    ...
    //pangolin::LoadGeometry("/home/mxy/Downloads/ORB_SLAM2-map_save_load_and_bin_voc/SpongeBob/spongebob.obj");
    glPopMatrix();
}
...
void ViewerAR::DrawArea(cv::Mat &im)
{
    float w = im.cols;
    float h = im.rows;
    float r = 50.0;
...
bool ViewerAR::IsNumber(const string &s)
{
    stringstream sin(s);
    double t;
    char p;
...
bool ViewerAR::SaveARPos(const string &sTpwPath)
{
    //string filename = "/home/mxy/Downloads/ORB_SLAM2-map_save_load_and_bin_voc/config/ARPos.yaml";
    cv::FileStorage SaveARPos(sTpwPath,cv::FileStorage::WRITE);
    SaveARPos<<"number"<<mnumber;
    for(int i=1; i<=mnumber; i++)
    {
        string name = "Tpw" + std::to_string(i);
        string name1 = static_cast<string>(name);
        if(mvpPlane[i])
        {
            if(!(mvpPlane[i]->Tpw.empty()))
            {
                mvTpw[i] = mvpPlane[i]->Tpw;
                cv::Mat Ttemp = mvpPlane[i]->Tpw.clone();
                SaveARPos<<name1<<Ttemp;
            }
        }
        else
        {
            cv::Mat Ttemp1;
            SaveARPos<<name1<<Ttemp1;
        }
    }
    SaveARPos.release();
    return true;
...
bool ViewerAR::LoadARPos(const string &sTpwPath)
{
    int num;
    //string filename = "/home/mxy/Downloads/ORB_SLAM2-map_save_load_and_bin_voc/config/ARPos.yaml";
    cv::FileStorage LoadARPos(sTpwPath,cv::FileStorage::READ);
    LoadARPos["number"]>>num;
    for(int i=1; i<=num; i++)
    {
        cv::Mat Ttemp;
        string name = "Tpw" + std::to_string(i);
        string name1 = static_cast<string>(name);
        LoadARPos[name1]>>Ttemp;
        if(!Ttemp.empty())
        {
            Plane* pPlane = new Plane(Ttemp);
            if(pPlane)
            {
                mvpPlane[i] = pPlane;
            }
...
//    for(std::size_t i=1; i<=mvTpw.size(); i++)
//    {
//        if(!mvTpw[i].empty())
//        {
//            cv::Mat Ttemp = mvTpw[i];
//            Plane* pPlane = new Plane(Ttemp);
//            if(pPlane)
//            {
//                mvpPlane[i] = pPlane;
