...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber sub = nodeHandler.subscribe("/usb_cam1/image_raw", 1, &ImageGrabber::GrabImage,&igb);///camera/image_raw /mynteye/left/image_raw /usb_cam1/image_raw
...
//     string sTfixedPath;
//     bool bTfixed = true;
//     bTfixed = static_cast<bool>((int)fSettings["Tfixed.state"]);
//     sTfixedPath = static_cast<string>((string)fSettings["Tfixed.path"]);
//     if(bTfixed)
//     {
//        cv::FileStorage fs(sTfixedPath, cv::FileStorage::WRITE);
...
//     }
//     
//     cv::FileStorage fMatrix(sTfixedPath, cv::FileStorage::READ);
//     cv::Mat tempMatrix;
//     fMatrix["cameraMatrix"] >> tempMatrix;
//     cout << "tempMatrix = " << endl << "" << tempMatrix << endl ;
    //test
    string sTfixedPath, sTcw1Path;
    bool bTfixed = true;
    bTfixed = static_cast<bool>((int)fSettings["Tfixed.state"]);
    sTfixedPath = static_cast<string>((string)fSettings["Tfixed.path"]);
    sTcw1Path = static_cast<string>((string)fSettings["Tcw1.path"]);
    //cout << "sTwc1Path " << sTwc1Path << endl;
    if(bTfixed)
    {
        viewerAR.SetTfixedFilePath(sTfixedPath, sTcw1Path);
    }
...
void ImageGrabber::GrabImage(const sensor_msgs::ImageConstPtr& msg)
{
    ...
    if(bRGB)
    {
        viewerAR.SetImagePose(imu,Tcw,state,vKeys,vMPs,Tcm,a,vIndices);
    }
    else
    {
